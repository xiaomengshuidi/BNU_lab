...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\程序设计\\2021\\taskSwitching\\taskSwitching_lastrun.py',
...
for thisTrial in trials:
    ...
    _key_resp_1_allKeys = []
    # keep track of which components have finished
    Image1Components = [image1, key_resp_1]
    ...
    while continueRoutine:
        ...
        if key_resp_1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_1.tStartRefresh + duration1-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_1.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp_1_allKeys):
                ...
                # was this correct?
                if (key_resp_1.keys == str(correctAns)) or (key_resp_1.keys == correctAns):
                    key_resp_1.corr = 1
                else:
                    key_resp_1.corr = 0
